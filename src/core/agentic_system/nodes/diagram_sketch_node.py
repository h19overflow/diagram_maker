...
def diagram_sketch_node(state: GraphState) -> GraphState:
    ...
    try:
        ...
        results_with_scores = asyncio.run(vector_store.search_with_scores(question, k=10))
        ...
        logger.info(f"Query relevance check - Max score: {max_score:.3f}, Avg score: {avg_score:.3f}, Threshold: {config.RELEVANCE_THRESHOLD}")
        ...
        structured_response = invoke_agent(question)["structured_response"]
...
if __name__ == "__main__":
    state = GraphState(user_input="I want to create a diagram of the human body")
