...
def needs_quotes(text: str) -> bool:
    """
    Check if a label needs to be wrapped in quotes due to special characters.

    Labels containing parentheses, brackets, or other special characters
    that conflict with Mermaid syntax should be wrapped in quotes.

    Args:
        text: Label text to check

    Returns:
        True if the label should be wrapped in quotes
    """
    if not text:
        return False

    # Check for characters that conflict with Mermaid syntax
    # Parentheses, brackets, and certain other characters need quoting
    special_chars = ["(", ")", "[", "]", "{", "}", "|", "#", ";", ":"]
...
def format_node_label(
    ...
    Returns:
        Tuple of (formatted label string, needs_quotes flag)

    Example:
        >>> format_node_label("Node Title", "Description text")
        ('Node Title<br/>Description text', False)
    ...
    if not description:
        ...
        label = f"{escaped_title}<br/>{escaped_description}"

    # Check if label needs quotes (check original text, not escaped)
    full_text = title if not description else f"{title}<br/>{description}"
    should_quote = needs_quotes(full_text)

    return (label, should_quote)
